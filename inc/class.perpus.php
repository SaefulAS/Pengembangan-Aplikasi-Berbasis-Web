...
class anggota extends perpus{
	public function create($nis,$nama,$tempat_lahir,$tgl_lahir,$jk,$jurusan,$thn_masuk){
		try {
		$stmt = $this->conn->prepare('INSERT INTO tbl_anggota(nis,nama,tempat_lahir,tgl_lahir,jk,jurusan,thn_masuk) VALUES(?,?,?,?,?,?,?)');
	...
	public function update($nis,$nis1,$nama,$tempat_lahir,$tgl_lahir,$jk,$jurusan,$thn_masuk){
		try {
		  $stmt = $this->conn->prepare("UPDATE tbl_anggota SET nis=:nis1, nama=:nama, tempat_lahir=:tempat_lahir, tgl_lahir=:tgl_lahir, jk=:jk, jurusan=:jurusan, thn_masuk=:thn_masuk WHERE nis=:nis");
	    $stmt->bindparam(":nis1",$nis1);
...
class transaksi extends perpus{
	...
	public function t_transaksi($buku,$id_mhs,$mhs,$tgl_pinjam,$tgl_kembali,$status,$ket){
		try {
		$stmt = $this->conn->prepare('INSERT INTO tbl_transaksi(judul,nis,nama,tgl_pinjam,tgl_kembali,status,ket) VALUES(?,?,?,?,?,?,?)');
